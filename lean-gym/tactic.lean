...
meta def postprocess_tactic_state (ts : tactic_state) : tactic string := do
  ...
  tactic.set_bool_option `pp.notation false,
  tactic.set_bool_option `pp.full_names true,
  tactic.set_bool_option `pp.unicode false,
  tactic.set_bool_option `pp.structure_projections false,
  options ← tactic.get_options,
  pure $ to_string (to_fmt (tactic_state.set_options ts options))
